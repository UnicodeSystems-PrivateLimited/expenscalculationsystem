<!DOCTYPE html>
<html>
<head>
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
</head>
<body style="margin:0px;">
<div style="max-width:600px;font-family:arial;margin: 20px auto;border:1px solid #ededed;font-size:14px;">
                        <div style="background: #eff6fc;border-radius: 3px 3px 0 0;border-bottom: 1px solid #d1e5f7;">
                            <span style="color: #fff;font-size: 36px;padding: 12px 0px 16px;display: inline-block;text-align: center;width: 100%;">
                                <span style="vertical-align: middle;"><img src="https://vataxcloud.com/resources/assets/images/vataxcloud-logo.png" style="max-width:176px;"></span>
                            </span>
                        </div>
                        <div style="font-size: 16px;color: #545454;padding: 25px 18px 10px;line-height: 1.5;font-family: sans-serif;/* background: #e4f1ff; */text-align: center;margin: 0px 15px;">
                     <strong style="clear:both;color: #076ecf;font-size: 24px;font-weight: 500;">Hello,</strong>
                            <div style="padding: 10px 14px 15px 14px;">
                            <p style="margin: 0; padding-bottom: 8px;"> Please find the attached Expense Excel file.</p>
                            </div>
                        
                        <div class="" style="display: block;width: 100%;padding: 20px 0 2px;text-align: center;color: #636060;font-size: 17px;margin: 0;font-family: sans-serif;/* border-top: 2px solid #dfdfdf; */margin-top: 30px;">
                        <strong style="clear:both;font-weight: 500;/* color: #076ecf; *//* font-size: 19px; */">Thanks</strong>
                        <br><span style=" color: #076ecf; font-size: 18px;">Vatax Cloud Team</span>
                        </div>
                </div>
                        
                    <div style="background: #f3f3f3;color: #606060;display: block;font-size: 12px;line-height: 18px;padding: 16px 8px 8px;text-align: center;border-top: 1px solid #e8e8e8;background: #EAEAEA;">
                            
                    <p style="color: #03a9f4; font-weight: 500; font-size: 14px;line-height: 20px;color: #666; width:100%;">Copyright © 2017 Vatax Cloud</p>
                                    
                        </div></div>
</body>
</html>
